...
//        outState.putParcelableArrayList("mindMapObjectList", ArrayList(mindMapObjectList))
    ...
    fun update(map: Map<String, MindMapObject>, categoryList: MutableList<Category>) {
        this.map = map
    ...
    companion object {
        @JvmStatic
        fun newInstance(mEventId: Int, map: Map<String, MindMapObject>, categoryList: MutableList<Category>) = GroupingResultFragment().apply {
            ...
            arguments?.putParcelableArrayList("categoryList", ArrayList(categoryList))
        }
    }
}
